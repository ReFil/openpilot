  ...
  for (int i = 0; i < parse_sigs.size(); i++) {
    ...
    if (!ignore_checksum) {
      if (sig.type == SignalType::HONDA_CHECKSUM && honda_checksum(address, dat) != tmp) {
        ...
        checksum_failed = true;
      } else if (sig.type == SignalType::OCELOT_COUNTER && ocelot_checksum(dat) != tmp) {
        checksum_failed = true;
      }
    }
    ...
    if (!ignore_counter) {
      if (sig.type == SignalType::HONDA_COUNTER || sig.type == SignalType::VOLKSWAGEN_COUNTER || sig.type == SignalType::PEDAL_COUNTER || sig.type == SignalType::OCELOT_COUNTER) {
